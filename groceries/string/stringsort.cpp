/*
 ...
 1. split strings into small files witch can be fully loaded into memory
 ...
 e.g.
 g++ stringsort.cpp -o stringsort // compile
 ./stringsort // execute( assume there is an out.txt which generated by genstring in the same folder )
...
int process(FILE * file) {
	...
	while (fscanf(file, "%s", str) != EOF) {
		...
		if (line % batchSize == 0 || tmpf == NULL) { // consider to start a new thread for each batch
			...
			ostringstream name;
			name << "tmp_0_" << num++ << ".txt";
			tmpf = fopen(name.str().c_str(), "w"); // 临时文件名为 tmp_x_x.txt，x为数字，从0开始
		}
